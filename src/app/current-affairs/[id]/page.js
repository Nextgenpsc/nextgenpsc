...
export default function CurrentAffairsDetail() {
  const params = useParams(); // expects route app/current-affairs/[id]/page.js
  const id = params?.id;
  ...
  const [bookmarkedPoints, setBookmarkedPoints] = useState([]);
  ...
  const loadBookmarkedPoints = async () => {
    ...
    try {
      ...
      console.error("Error loading bookmarked points:", e);
    }
  };

  const addBookmark = async (point) => {
    if (bookmarkedPoints.includes(point)) return;
    if (!isAuthenticated) {
      const local = JSON.parse(localStorage.getItem(`current-affairs-${id}-bookmarks`) || "[]");
      const updated = [...local, point];
      localStorage.setItem(`current-affairs-${id}-bookmarks`, JSON.stringify(updated));
      setBookmarkedPoints(updated);
      toast({ title: "Bookmark saved", description: "Added to your bookmarks" });
      return;
    }
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        toast({ title: "Authentication required", description: "Please sign in to bookmark", variant: "destructive" });
        return;
      }
  ...
  const removeBookmark = async (point, index) => {
    if (!isAuthenticated) {
      const local = JSON.parse(localStorage.getItem(`current-affairs-${id}-bookmarks`) || "[]");
  ...
  const exportBookmarksToPDF = () => {
    if (bookmarkedPoints.length === 0) {
      toast({ title: "No bookmarks", description: "Add some bookmarks first", variant: "destructive" });
      return;
    }
    const doc = new jsPDF();
    ...
    bookmarkedPoints.forEach((note, idx) => {
      const wrapped = doc.splitTextToSize(`${idx + 1}. ${note}`, 180);
  ...
  const exportBookmarksToDOCX = async () => {
    ...
    const doc = new Document({
      sections: [
        {
          children: [
            new Paragraph({ children: [new TextRun({ text: `Current Affairs Bookmarks - ${article?.title || ""}`, bold: true, size: 28 })] }),
            ...bookmarkedPoints.map((note, idx) => new Paragraph({ children: [new TextRun({ text: `${idx + 1}. ${note}`, size: 24 })] })),
          ],
        },
      ],
    });
  ...
  const toggleBookmark = async () => {
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        toast({ title: "Authentication required", description: "Please sign in to bookmark articles", variant: "destructive" });
        return;
      }
      if (isBookmarked && bookmarkId) {
        const { error } = await supabase.from("current_affairs_bookmarks").delete().eq("id", bookmarkId);
        if (error) throw error;
        setIsBookmarked(false);
        setBookmarkId(null);
  ...
  // ---- SEO bits (no hooks needed) ----
  const details = article?.details || {};
  const seoTitle = details?.topic ? `${details.topic} — Current Affairs` : "Current Affairs — Detail";
  const seoDesc =
    article?.summary ||
    details?.background?.summary ||
    "UPSC-focused current affairs article with background, significance, challenges, and questions.";
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "";
  const canonicalUrl = siteUrl ? `${siteUrl}/current-affairs/${id}` : "";
  ...
  const jsonLd = {
    ...
    author: { "@type": "Organization", name: process.env.NEXT_PUBLIC_SITE_NAME || "YourSite" },
  ...
  return (
    <>
      <Head>
        ...
        {details?.image ? <meta property="og:image" content={details.image} /> : null}
        {canonicalUrl ? <meta property="og:url" content={canonicalUrl} /> : null}
        {/* Twitter */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={seoTitle} />
        <meta name="twitter:description" content={seoDesc} />
        {details?.image ? <meta name="twitter:image" content={details.image} /> : null}
        {/* JSON-LD */}
      ...
      {showBookmarks && (
        <div
          className={`fixed bg-card border border-border rounded-lg shadow-2xl z-40 overflow-hidden ${
            isMobile ? "bottom-24 left-4 right-4 max-h-[calc(100vh-12rem)]" : "top-20 right-6 w-80 max-h-96"
          }`}
        >
          <div className="p-4 border-b border-border bg-primary/5 flex justify-between items-center">
            ...
            <Button onClick={() => setShowBookmarks(false)} size="sm" variant="ghost" className="h-6 w-6 p-0">
              <X className="h-4 w-4" />
            </Button>
          </div>
          <div className="overflow-y-auto max-h-60 p-2">
            {bookmarkedPoints.length === 0 ? (
              <p className="text-muted-foreground text-sm p-4 text-center">No bookmarks yet. Click on any point to save it.</p>
            ) : (
      ...
      <main className="container mx-auto px-4 py-8 pb-24">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <Button variant={isBookmarked ? "default" : "outline"} size="sm" onClick={toggleBookmark} disabled={loading}>
            {isBookmarked ? (
              <>
        ...
        <div className="mb-8">
          ...
          {details?.image && <img src={details.image} alt={details.topic} className="w-full h-64 object-cover rounded-lg mb-6" />}
        </div>
                  ...
                  <div className="grid gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        <strong>Location:</strong> <BookmarkableText text={details.background.location} />
                      </span>
                    </div>
                    {details.background.related_conflict && (
                      <div className="flex items-start gap-2">
                        ...
                        <span className="text-sm">
                          <strong>Related Event:</strong> <BookmarkableText text={details.background.related_conflict} />
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    <ul className="space-y-1">
                      {details.exercise_details.objectives?.map((objective, index) => (
                        <li key={index} className="text-sm text-muted-foreground">
                          • <BookmarkableText text={objective} />
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="grid gap-2">
                      <p className="text-sm">
                        <strong>Phase 1:</strong> {details.sudarshan_chakra.timeline?.phase1}
                      </p>
                      <p className="text-sm">
                        <strong>Phase 2:</strong> {details.sudarshan_chakra.timeline?.phase2}
                      </p>
                    </div>
                  </div>
                  <div>
                    ...
                    <ul className="space-y-1">
                      {details.sudarshan_chakra.components?.map((component, index) => (
                        <li key={index} className="text-sm text-muted-foreground">
                          • <BookmarkableText text={component} />
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1">
                        {details.sudarshan_chakra.recent_milestones.map((milestone, index) => (
                          <li key={index} className="text-sm text-muted-foreground">
                            • <BookmarkableText text={milestone} />
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {details.challenges.map((challenge, index) => (
                      <li key={index} className="text-sm text-muted-foreground">
                        • <BookmarkableText text={challenge} />
                      </li>
                    ))}
                  </ul>
                </CardContent>
