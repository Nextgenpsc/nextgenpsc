...
export default function AdminPage() {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          max_score: count * 2, // e.g., 2 marks per question
        })
        .eq("id", testId);
      ...
      setTestSeries((prev) =>
        prev.map((t) =>
          t.id === testId ? { ...t, total_questions: count, max_score: count * 2 } : t
        )
      );
    } catch (error) {
  ...
  const deleteQuestion = async (questionId) => {
    try {
      ...
      if (selectedTestId) {
        await updateTestQuestionCount(selectedTestId, Math.max(0, questions.length - 1));
      }
  ...
  const handleEditTest = async () => {
    ...
    try {
      ...
      setTestSeries((prev) => prev.map((t) => (t.id === editingTest.id ? editingTest : t)));
      setEditingTest(null);
                        ...
                        {newTest.test_type === "chapter" && (
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setNewTest({ ...newTest, chapter_name: e.target.value })
                              }
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                setEditingTest({ ...editingTest, subject_id: value })
                              }
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="Select subject" />
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                setEditingTest({ ...editingTest, test_type: value })
                              }
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="Select type" />
                        ...
                        {editingTest.test_type === "chapter" && (
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setEditingTest({ ...editingTest, chapter_name: e.target.value })
                              }
                              placeholder="Enter chapter name"
                            />
                          </div>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    will also delete all questions associated with this test
                  ...
                  {editingQuestion && (
                    <div className="space-y-4">
                      ...
                      <div>
                        <Label>Options</Label>
                        <div className="space-y-2">
                          {(Array.isArray(editingQuestion.options) ? editingQuestion.options : []).map(
                            (option, index) => (
                              <Input
                                key={index}
                                value={option || ""}
