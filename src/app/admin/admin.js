...
export default function Admin() {
  ...
  const handleAddTest = async () => {
    try {
      ...
      const { data, error } = await supabase.from("test_series").insert([testData]).select().single();
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("test_questions").insert([questionData]).select().single();
  ...
  const updateTestQuestionCount = async (testId, count) => {
    try {
      ...
      setTestSeries(
        testSeries.map((test) =>
          test.id === testId ? { ...test, total_questions: count, max_score: count * 2 } : test
        )
      );
    } catch (error) {
  ...
  const deleteTest = async (testId) => {
    try {
      const { error } = await supabase.from("test_series").delete().eq("id", testId);
  ...
  const deleteQuestion = async (questionId) => {
    try {
      ...
      if (selectedTestId) {
        await updateTestQuestionCount(selectedTestId, newQuestions.length);
      }
  ...
  const handleEditTest = async () => {
    ...
    try {
      ...
      setTestSeries(testSeries.map((test) => (test.id === editingTest.id ? editingTest : test)));
      setEditingTest(null);
          ...
          <Tabs defaultValue="tests" className="space-y-6">
                    ...
                    <Card key={test.id}>
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={() => setSelectedTestId(test.id)}>
                              Manage Questions
                            </Button>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>Are you sure you want to delete "{test.title}"?</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <div className="font-medium">{upscSubjects.find((s) => s.id === test.subject_id)?.name || "General"}</div>
                          </div>
                          <div>
            ...
            <TabsContent value="questions">
              <div className="space-y-6">
                      ...
                      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-6">
                          ...
                          <div>
                            ...
                            <Input value={newQuestion.topic || ""} onChange={(e) => setNewQuestion({ ...newQuestion, topic: e.target.value })} placeholder="Enter topic" />
                          </div>
                ...
                {selectedTestId && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold">Questions for: {testSeries.find((t) => t.id === selectedTestId)?.title}</h3>
                    {questions.length === 0 ? (
                            ...
                            <CardHeader>
                              <div className="flex items-start justify-between">
                                <CardTitle className="text-base">
                                  ...
                                  <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: question.question_text }} />
                                </CardTitle>
                                <div className="flex gap-2">
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>Are you sure you want to delete this question?</AlertDialogDescription>
                                      </AlertDialogHeader>
                                      <AlertDialogFooter>
                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                              ...
                              <div className="space-y-2">
                                    ...
                                    ? question.options.map((option, optIndex) => (
                                        <div
                                          ...
                                          className={`p-2 rounded border ${
                                            option === question.correct_answer ? "bg-green-50 border-green-200 text-green-800" : "bg-gray-50 border-gray-200"
                                          }`}
                                        >
                                ...
                                {question.explanation && (
                                  <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                                    ...
                                    <div className="prose prose-sm max-w-none mt-2" dangerouslySetInnerHTML={{ __html: question.explanation }} />
                                  </div>
                                )}
                      ...
                      <div className="space-y-6">
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              const resetOptions = value === "true_false" ? ["True", "False"] : editingQuestion.options;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            {(editingQuestion.question_type || "multiple_choice") === "true_false" ? (
                              <Select value={editingQuestion.correct_answer} onValueChange={(value) => setEditingQuestion({ ...editingQuestion, correct_answer: value })}>
                                <SelectTrigger>
                                  <SelectValue placeholder="Select correct answer" />
                                </SelectTrigger>
                                <SelectContent>
                        ...
                        <div>
                          <Label>Topic (Optional)</Label>
                          <Input value={editingQuestion.topic || ""} onChange={(e) => setEditingQuestion({ ...editingQuestion, topic: e.target.value })} placeholder="Enter topic" />
                        </div>
