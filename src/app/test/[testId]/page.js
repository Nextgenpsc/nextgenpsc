...
export default function TestSeriesTestPage() {
  ...
  const getPercentageForAttempt = (attempt) =>
  ...
  const handleStartTest = () => {
    const durationMins =
      testSeries?.duration && testSeries.duration > 0
        ? testSeries.duration
  ...
  const storeSectionPerformance = async (testAttemptId) => {
  ...
  const storeWeakSections = async (testAttemptId) => {
    if (!user || !testSeries) return;
        ...
        <div className="max-w-2xl mx-auto">
          <Card className="text-center">
            <CardHeader className="pb-4">
              ...
              <p className="text-muted-foreground">You have successfully completed the test</p>
            </CardHeader>

            <CardContent className="space-y-6">
              <div className="p-6 bg-muted/50 rounded-lg">
                <div className={`text-3xl font-bold mb-2 ${getScoreColor(percentage)}`}>{percentage}%</div>
                            ...
                            <div className="font-bold">
                              {Math.round(
                                previousAttempts.reduce(
                                  (sum, a) => sum + getPercentageForAttempt(a),
                                  0
